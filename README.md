...
- 首页显示今日建议摄入、已摄入、剩余热量
- 拍照/上传食物图片，AI 识别食物、重量、热量和三大营养素
- 配置 `ARK_API_KEY` 和 `ARK_VISION_MODEL` 后，拍照会优先调用豆包/火山方舟视觉模型
- 也可配置 `OPENAI_API_KEY` 作为备用视觉识别
- 未配置可用视觉模型时，不再随机假识别，默认生成“待确认食物”，用户可快速选择常见食物或手动修改
- 确认记录后自动扣除今日剩余热量
- 根据剩余热量推荐中国常见食物
- 历史记录只显示日期、总热量、是否超标
- 每日体重记录和最近 7 天趋势
